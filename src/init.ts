...
(async function main() {
  ...
  const rollupExists = existsSync(dir("rollup.config.js"));
  ...
  const answers = await inquirer.prompt([
    ...
    },
    {
      type: "confirm",
      name: "jest",
      message: "Would you like me to set up testing using Jest?",
      default: true,
    },
    {
      type: "confirm",
    ...
    },
    {
      ...
      message: (
        rollupExists
          ...
          : "Would you like me to create a rollup.config.js file?"
      ),
    },
    {
      type: "confirm",
  ...
  if (answers.createRollupConfig) {
    await checkpoint("Creating rollup.config.js file using this library", async () => {
      writeFileSync(dir("rollup.config.js"),
...
export default config({
  ...
  if (answers.createTsConfig) {
    await checkpoint("Create tsconfig.json file with some opinionated settings", async () => {
